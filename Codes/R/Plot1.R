...
df <- subset(Data, select=c(CIT, expansion, YEAR, HICOV))
...
plot1<-ggplot(data=insurance_mean, aes(x=CITG, y=HICOV, fill=ACA)) +
...
df_sub <- Data[, c("CIT", "expansion", "YEAR", "HICOV", grep("PWGTP", names(Data), value = TRUE))]
...
# Create the bar graph
plot1w<-ggplot(data=insurance_mean, aes(x=CITG, y=HICOV, fill=ACA)) +
  geom_bar(stat="identity", position=position_dodge()) +
  facet_grid( ~ expansion) +
  scale_y_continuous(limits=c(0,1), expand=c(0,0), breaks=seq(0,1,0.1))+
