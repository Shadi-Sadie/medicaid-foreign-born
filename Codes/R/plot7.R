# Code for creating plot 6 insurance rate by race and nativity

## Insurance thriygh employer
df <- subset(Data, select=c(RACE,ETHN ,NATIVITY,expansion, YEAR, HINS1))
...
insurance_mean <- aggregate(HINS1 ~ NATIVITY + RACE + expansion, data=df, FUN=mean)


# Create the bar graph
plot7<-ggplot(data=insurance_mean, aes(x=RACE, y=HINS1, fill=NATIVITY)) +
