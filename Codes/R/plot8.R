# Code for creating plot 8  self purchased insurance rate by race and nativity

## Insurance thriygh employer
df <- subset(Data, select=c(RACE,ETHN ,NATIVITY,expansion, YEAR, HINS2))
...
insurance_mean <- aggregate(HINS2 ~ NATIVITY + RACE + expansion, data=df, FUN=mean)


# Create the bar graph
plot8<-ggplot(data=insurance_mean, aes(x=RACE, y=HINS2, fill=NATIVITY)) +
        geom_bar(stat="identity", position=position_dodge()) +
        facet_grid( ~ expansion) +
        scale_y_continuous(limits=c(0,1), expand=c(0,0), breaks=seq(0,1,0.1))+
        scale_fill_brewer(palette="Pastel1")+ # specify the colors you want
        labs(x="Race", y="Directly Purchased Insurance Rate", title="Insurance Rates by Race and Nativity Status and Expansion Status") +
        #theme_classic()
