...
# This is the code for creating the graph5(Insurance rate by race and nativity)


df <- subset(Data, select=c(RACE,ETHN ,NATIVITY,expansion, YEAR, HICOV))
...
insurance_mean <- aggregate(HICOV ~ NATIVITY + RACE + expansion, data=df, FUN=mean)


# Create the bar graph
plot5<-ggplot(data=insurance_mean, aes(x=RACE, y=HICOV, fill=NATIVITY)) +
