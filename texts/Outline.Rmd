...
#FBC<- Forgn[Forgn$CIT %in%  c("Naturalized-citizen"), ]
...
Event4 = feols(HINS4 ~ i(ttot, expansion, ref = -1)
               +ADA+ IPC+ UnempR+.[controlwof] + .[forcont]|                 
...
Event7 = feols(HINS1 ~ i(ttot, expansion, ref = -1)+ADA+ IPC+ UnempR+.[controlwof] + .[forcont]|                 
                       ST + YEAR,                             ## FEs
               vcov= "hetero" ,
               weights = ~PWGTP,
               data = NATV)
...
Event10 = feols(HINS2 ~ i(ttot, expansion, ref = -1)+ADA+ IPC+ UnempR+.[controlwof] + .[forcont]|                 
                       ST + YEAR,                             ## FEs
               vcov= "hetero" ,
               weights = ~PWGTP,
               data = NATV)
...
figurewtf2<-ggarrange(Uninsured, Medicaid , Employer, DirPurchs, 
                   ncol = 2, nrow = 2)

annotate_figure(figurewtf2,
                top = text_grob("Uninsured Rate", face = "bold", size = 14),
